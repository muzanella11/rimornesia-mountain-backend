...
class SeederDistricts(object):
    ...
    def __init__(self):
        super(SeederDistricts, self).__init__()
    ...
    def run(self):
        ...
        for target_list in districts_list:
            insert_districts = districts_list.get(target_list)
            
            action['districts_{}'.format(target_list)] = {
                'table_name': 'districts',
                'action': self.ACTION.get('insert'),
                'command': (
                    "{} {} {}".format(self.lock_table(), insert_districts, self.unlock_table())
                )
            }
    ...
    def lock_table(self):
        return "LOCK TABLES `districts` WRITE;"

    def unlock_table(self):
        return "UNLOCK TABLES;"

    def set_queries(self, command):
        self.counter = self.counter + 1
        
        self.QUERIES['districts_{}'.format(self.counter)] = (
            command
        )
